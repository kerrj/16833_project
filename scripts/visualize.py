...
def visualize_scan(pts,frame):
  R=np.array([[np.cos(frame[2,0]),-np.sin(frame[2,0])],[np.sin(frame[2,0]),np.cos(frame[2,0])]])
  T=frame[0:2,:]
...
while True:
  ...
  while(True):
    ...
    if logline[1]=="S":break
  logline=logline[3:]
  pts = np.array([[float(logline[2*i]),float(logline[2*i+1])] for i in range(len(logline)//2)])
