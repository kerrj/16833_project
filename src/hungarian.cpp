...
//
// (Note: end value is cached, so fo(i, function()) will only have function
// called once.)
...
struct LeftEdge {
  ...
  const bool operator<(const LeftEdge& otherEdge) const {
    return right < otherEdge.right ||
           (right == otherEdge.right && cost < otherEdge.cost);
  }
};

const std::vector<int> hungarianMinimumWeightPerfectMatching(
    const int n, const std::vector<WeightedBipartiteEdge> allEdges) {
  // Edge lists for each left node.
  const std::unique_ptr<std::vector<LeftEdge>[]> leftEdges(
      new std::vector<LeftEdge>[n]);

  // region Edge list initialization

  // Initialize edge lists for each left node, based on the incoming set of
  // edges. While we're at it, we check that every node has at least one
  // associated edge. (Note: We filter out the edges that invalidly refer to a
  // node on the left or right outside [0, n).)
  {
    ...
    // Probably unnecessary, but reserve the required space for each node, just
    // because?
    fo(i, n) { leftEdges[i].reserve(leftEdgeCounts[i]); }
  }
  ...
  // Sort the edge lists, and remove duplicate edges (keep the edge with
  // smallest cost).
  fo(i, n) {
  ...
  // These hold "potentials" for nodes on the left and nodes on the right, which
  // reduce the costs of attached edges. We maintain that every reduced cost,
  // cost[i][j] - leftPotential[i] - leftPotential[j], is greater than zero.
  ...
  // Note: We're guaranteed by the above code that at every node on the left and
  // right has at least one edge.
  ...
  fo(i, n) {
    ...
    // This is as high as we can take it without creating an edge with zero
    // reduced cost.
    leftPotential[i] = smallestEdgeCost;
  }

  // Second, we raise the potentials on the right as high as we can for each
  // node. We do the same as with the left, but this time take into account that
  // costs are reduced by the left potentials. This guarantees that each node on
  // the right has at least one "tight" edge.
  ...
  // Following initialization, we maintain the invariant that these are at the
  // start of the node's edge list.
  ...
  // We will be interested in the subgraph induced by the tight edges, so we
  // partition the edge lists for each left node accordingly, moving the tight
  // edges to the start.
  ...
  fo(i, n) {
    ...
    fo(edgeIndex, edges.size()) {
      const LeftEdge& edge = edges[edgeIndex];
      int reducedCost =
          edge.cost - leftPotential[i] - rightPotential[edge.right];
      if (reducedCost == 0) {
  ...
  // We maintain an (initially empty) partial matching, in the subgraph of tight
  // edges.
  ...
  // When an augmenting path doesn't exist, we update the potentials so that an
  // edge between the area we can reach and the unreachable nodes on the right
  // becomes tight, giving us another edge to explore.
  //
  // We proceed in this fashion until we can't find more augmenting paths or add
  // edges. At that point, we either have a min-weight perfect matching, or no
  // matching exists.

  // region Inner loop state variables

  // One point of confusion is that we're going to cache the edges between the
  // area we've explored that are "almost tight", or rather are the closest to
  // being tight. This is necessary to achieve our O(N^3) runtime.
  //
  // rightMinimumSlack[j] gives the smallest amount of "slack" for an unreached
  // node j on the right, considering the edges between j and some node on the
  // left in our explored area.
  //
  ...
  while (currentMatchingCardinality < n) {
    ...
    // Note: We need to clear the nodes so that we can notice when there aren't
    // any edges available.
    ...
    // By heuristic, we pick the node with fewest tight edges, giving the BFS an
    // easier time. (The asymptotics don't care about this, but maybe it helps.
    // Eh.)
    {
      int minimumTightEdges = oo;
      fo(i, n) {
        if (leftMatchedTo[i] == UNMATCHED &&
            leftTightEdgesCount[i] < minimumTightEdges) {
    ...
    while (endingRightNode == UNMATCHED) {
      ...
      while (endingRightNode == UNMATCHED && !leftNodeQueue.empty()) {
        // Implementation note: this could just as easily be a DFS, but a BFS
        // probably has less edge flipping (by my guess), so we're using a BFS.

        const int i = leftNodeQueue.front();
        leftNodeQueue.pop_front();

        std::vector<LeftEdge>& edges = leftEdges[i];
        // Note: Some of the edges might not be tight anymore, hence the awful
        // loop.
        for (int edgeIndex = 0; edgeIndex < leftTightEdgesCount[i];
             ++edgeIndex) {
          const LeftEdge& edge = edges[edgeIndex];
          ...
          if (matchedTo == UNMATCHED) {
            ...
            // No match found, but a new left node is reachable. Track how we
            // got here and extend BFS queue.
        ...
        if (endingRightNode == UNMATCHED) {
          ...
          range(edgeIndex, leftTightEdgesCount[i], edges.size()) {
            ...
            if (rightMatchedTo[j] == UNMATCHED ||
                !leftSeen[rightMatchedTo[j]]) {
              // This edge is to a node on the right that we haven't reached
              // yet.
              ...
              if (reducedCost < rightMinimumSlack[j]) {
                ...
                // One array instead of 3. But I can't think of something else.
                // So it goes.
      ...
      if (endingRightNode == UNMATCHED) {
        ...
        if (minimumSlackRightNode == UNMATCHED ||
          ...
          // This means that the node the BFS started at, which is an unmatched
          // left node, cannot reach the right - i.e. it will be impossible to
        ...
        fo(j, n) {
          if (rightMatchedTo[j] == UNMATCHED || !leftSeen[rightMatchedTo[j]]) {
            ...
            // If the slack hit zero, then we just found ourselves a new tight
            // edge.
            if (rightMinimumSlack[j] == 0) {
              ...
              // If we haven't already encountered a match, we follow the edge
              // and update the BFS queue. It's possible this edge leads to a
              // match. If so, we'll carry on updating the tight edges, but
              // won't follow them.
              if (endingRightNode == UNMATCHED) {
                // We're contemplating the consequences of following (i, j), as
                // we do in the BFS above.
                rightBacktrack[j] = i;
                int matchedTo = rightMatchedTo[j];
                if (matchedTo == UNMATCHED) {
                  // Match found!
    ...
    // At this point, we've found an augmenting path between startingLeftNode
    // and endingRightNode. We'll just use the backtracking info to update our
    // match information.
